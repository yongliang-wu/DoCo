...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_batch(
        ...
        """
        Batched version of step function. Predict samples at previous timesteps for a batch of different current timesteps.

        Args:
            model_output (`torch.FloatTensor`): direct output from learned diffusion model.
            timesteps (`torch.IntTensor`): current timesteps in the diffusion chain.
            sample (`torch.FloatTensor`):
                current instance of sample being created by diffusion process.
            generator: random number generator.
            return_dict (`bool`): option for returning tuple rather than DDPMSchedulerOutput class
        ...
        Returns:
            [`~schedulers.scheduling_utils.DDPMSchedulerOutput`] or `tuple`
        """
        # Ensure all inputs are same batch size
        ...
        num_inference_steps = self.num_inference_steps if self.num_inference_steps else self.config.num_train_timesteps
        prev_timesteps = timesteps - self.config.num_train_timesteps // num_inference_steps
        ...
        alpha_prod_t_prev = torch.where(
            ...
            self.one.expand_as(prev_timesteps)
        )
        ...
        # Ensure tensors are properly shaped for broadcasting
        ...
        current_alpha_t = current_alpha_t.view(-1, 1, 1, 1)
        current_beta_t = current_beta_t.view(-1, 1, 1, 1)
        ...
        if mask.sum() > 0:
            ...
            if self.variance_type == "fixed_small_log":
                variance = self._get_variance(timesteps, predicted_variance=predicted_variance).view(-1, 1, 1, 1) * variance_noise
            elif self.variance_type == "learned_range":
                variance = self._get_variance(timesteps, predicted_variance=predicted_variance).view(-1, 1, 1, 1)
                variance = torch.exp(0.5 * variance) * variance_noise
            else:
                variance = (self._get_variance(timesteps, predicted_variance=predicted_variance).sqrt()).view(-1, 1, 1, 1) * variance_noise
